...
| <a name="input_config_non_main_exclude_resource_types"></a> [config\_non\_main\_exclude\_resource\_types](#input\_config\_non\_main\_exclude\_resource\_types) | Global Resource types that will be excluded from config\_limit\_resource\_types in a non-main regions | `list(string)` | no |
...
| <a name="input_securityhub_policy_assignments"></a> [securityhub\_policy\_assignments](#input\_securityhub\_policy\_assignments) | Assignments of Security Hub configuration policies to target accounts or OUs. Note: this only works if delegated admin account is used. | <pre>map(object({<br/>    target_id   = string,<br/>    policy_name = string,<br/>  }))</pre> | no |
| <a name="input_ssm_baseline_enabled"></a> [ssm\_baseline\_enabled](#input\_ssm\_baseline\_enabled) | Boolean whether ssm-baseline is enabled. Defaults to true. | `bool` | no |
| <a name="input_ssm_documents_public_sharing_permission_disabled"></a> [ssm\_documents\_public\_sharing\_permission\_disabled](#input\_ssm\_documents\_public\_sharing\_permission\_disabled) | Whether AWS SSM should block public sharing of SSM documents in ALL regions. Defaults to true. | `bool` | no |
