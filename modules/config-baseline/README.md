...
| <a name="input_daily_recording"></a> [daily\_recording](#input\_daily\_recording) | Enable DAILY recording (as opposed to CONTINUOUS). | `bool` | no |
...
| <a name="input_limit_resource_types"></a> [limit\_resource\_types](#input\_limit\_resource\_types) | Only record changes for listed resource types. Records all if empty. | `list(string)` | no |
